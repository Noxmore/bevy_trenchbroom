...
First, enable the `rapier` or `avian32`/`avian64` feature on the crate, then either call `convex_collider` or `trimesh_collider` on your class's `SpawnHooks` to create the respective type of collider(s) with said geometry.
