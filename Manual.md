...
For compiling the `.map` into a `.bsp`, many of the compiler's defaults are specifically for compiling for the Quake engine, which has quite a few more limitations than Bevy, so we'll have to remove some with command-line arguments. Here are some I recommend:

`qbsp -qbism -nosubdivide -nosoftware -path assets -notex`
- `-qbism` - Uses a version of the Quake 2 BSP38 format with expanded limits.
...
- `-bouncecolorscale 1` - Uses texture colors for bounce lighting. For example, a very green wall will make nearby surfaces green when illuminated.
- `-bouncestyled 1` - Lets animated lights cause indirect lighting.
- `-dirt 1` - Enables "dirt mapping". Basically just baked ambient occlusion.
- `-phong 1` - Softens lighting on corners less than 90 degrees.

If you want to more finely tune your lighting, check out the [ericw-tools documentation](https://ericw-tools.readthedocs.io/en/latest/light.html) or look through your worldspawn entity if it uses `BspWorldspawn` as a base class.
...
## Animated Lighting
Each light can specify a [lighting style](qbsp::data::lighting::LightmapStyle) which governs the pattern of animation that light produces over time, affecting both lightmaps, and irradiance volumes.
...
Notice we start at one here. The convention is to reserve 0 for unanimated lighting ([`LightmapStyle::NORMAL`](qbsp::data::lighting::LightmapStyle::NORMAL)).
