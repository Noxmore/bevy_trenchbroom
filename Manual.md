...
Currently, PVS (potentially visible set) data generated by the `vis` tool isn't used.

At the time of writing, Bevy's default [`StandardMaterial::lightmap_exposure`](bevy::prelude::StandardMaterial::lightmap_exposure) makes lightmaps completely invisible.
The [`TrenchBroomConfig::lightmap_exposure`](bevy_trenchbroom::config::TrenchBroomConfig::lightmap_exposure) setting automatically sets it to a reasonable value on any `StandardMaterial` loaded.

You can also change [`TrenchBroomConfig::bsp_parse_settings`](bevy_trenchbroom::config::TrenchBroomConfig::bsp_parse_settings) and [`compute_lightmap_settings`](bevy_trenchbroom::config::TrenchBroomConfig::compute_lightmap_settings) to further configure how BSPs are loaded.
