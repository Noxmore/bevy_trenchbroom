...
impl BrushPlane {
    ...
    pub fn project(&self, point: DVec3) -> DVec2 {
        ...
        let x_normal = if x_normal == DVec3::ZERO {
            DVec3::X
        } else {
            x_normal
        };
        ...
        Self {
            surfaces: brush
                ...
                .map(|surface| BrushSurface {
                    plane: BrushPlane::from_triangle(
                        surface
                            .half_space
                            .map(|half_space| config.to_bevy_space_f64(DVec3::from(half_space))),
                    ),
                    ...
                    uv: BrushUV {
                        ...
                        axes: surface.alignment.axes.map(|axes| {
                            axes.map(|axes_vec| config.to_bevy_space_f64(DVec3::from(axes_vec)))
                        }),
                    },
                })
                .collect(),
...
pub fn generate_mesh_from_brush_polygons(
    ...
    let [vertices, normals] =
        [vertices, normals].map(|x| x.into_iter().map(|x| x.as_vec3()).collect::<Vec<Vec3>>());
