...
impl BrushPlane
{
	...
	pub fn from_triangle(tri: [DVec3; 3]) -> Self {
		...
		Self { normal, distance: -normal.dot(tri[0]) }
	}
...
impl Brush
{
	...
	pub(crate) fn from_quake_util(brush: &quake_util::qmap::Brush) -> Self {
		Self {
			surfaces: brush.iter().map(|surface|
				BrushSurface {
					plane: BrushPlane::from_triangle(surface.half_space.map(|half_space| DVec3::from(half_space).trenchbroom_to_bevy_space())),
					...
					uv: BrushUV {
						...
						axes: surface.alignment.axes.map(|axes| axes.map(|axes_vec| DVec3::from(axes_vec).trenchbroom_to_bevy_space())),
					}
				},
			).collect()
		}
	...
	pub fn polygonize(&self) -> Vec<BrushSurfacePolygon>
	{
		...
		for ((s1_i, s1), (s2_i, s2), (s3_i, s3)) in self.surfaces.iter().enumerate().tuple_combinations() {
			if let Some(intersection) = BrushPlane::calculate_intersection_point([&s1.plane, &s2.plane, &s3.plane])
			{
				...
				if !self.contains_point(intersection) { continue }
		...
		vertex_map.into_iter().map(|(surface_index, vertices)| BrushSurfacePolygon::new(self.surfaces[surface_index].clone(), vertices)).collect()
	}
}
		...
		if vertices.len() > 2
		{
			...
			
			{
				...
				rest.sort_unstable_by_key(|vertex| {
					if *vertex == vert_1 { return FloatOrd(0.0) }
					// Make sure duplicates of vert_0 are at the start of the list for deduplication
					if vertex.almost_eq(vert_0, Self::VERTEX_PRECISION_MARGIN) { return FloatOrd(f64::NEG_INFINITY) }
					let vertex_vector = *vertex - vert_0;

					FloatOrd(vertex_vector.cross(starting_vector).dot(surface.plane.normal).atan2(starting_vector.dot(vertex_vector)))
				});
			}
...
pub fn generate_mesh_from_brush_polygons(faces: &[&BrushSurfacePolygon], config: &TrenchBroomConfig) -> Mesh
{
	let texture_size = if faces.is_empty() { Vec2::ONE } else {
		UVec2::from(image::image_dimensions(config.assets_path.join(format!("materials/{}.png", &faces[0].surface.material))).unwrap_or((1, 1))).as_vec2()
	};
