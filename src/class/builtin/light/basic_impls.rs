...
impl CombinedLight {
	pub fn spawn_hook(view: &mut QuakeClassSpawnView) -> anyhow::Result<()> {
		...
		let bsp_light = entity_ref
			.get::<BspLight>()
			.ok_or_else(|| anyhow!("No BspLight found for mixed light implementation during spawn hook!"))?;
		let mixed_light = entity_ref
			.get::<MixedLight>()
			.ok_or_else(|| anyhow!("No MixedLight found for mixed light implementation during spawn hook!"))?;
