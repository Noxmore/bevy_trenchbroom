...
lazy_static! {
	pub static ref MATERIAL_PROPERTIES_CACHE: Mutex<HashMap<PathBuf, MaterialProperties>> = default();
}
...
impl MaterialProperties {
	...
	pub fn load(path: impl AsRef<Path>) -> MaterialProperties {
		...
		MATERIAL_PROPERTIES_CACHE.lock().unwrap().entry(path.to_owned()).or_insert_with(|| 
			ron::from_str(&fs::read_to_string(path).unwrap_or("()".into())).unwrap_or(MaterialProperties::default())
		).clone()
	}
}
