use bevy::{asset::{io::Reader, AssetLoader, AsyncReadExt, LoadContext}, utils::BoxedFuture};
...
impl AssetLoader for MapLoader
{
	...
	) -> BoxedFuture<'a, Result<Self::Asset, Self::Error>>
	{
		Box::pin(async move
		{
			...
			{
				let properties = ent.edict.into_iter()
			...
			if map.worldspawn().is_none() {
				return Err(io::Error::new(io::ErrorKind::InvalidInput, "worldspawn not defined"));
			}
